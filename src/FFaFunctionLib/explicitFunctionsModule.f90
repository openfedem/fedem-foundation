...
!> @file explicitFunctionsModule.f90
!> @brief Collection of explicit math functions.

!!==============================================================================
!> @brief Module with explicit math functions.
!>
!> @details This module contains a collection of Fortran functions for
!> evaluating explicit math functions. It consists of the following functions:
!>
!> -# FUNC1  : SIN-function
!> -# FUNC2  : Two SIN-functions summed, either TMAX or TMIN given
...
!>
!> Originally, this module was writtene by Ole Iver Sivertsen (OIS) in 1984,
!> and later converted to Fortran90 by Bjorn Haugen in 1999.
...
module ExplicitFunctionsModule
  ...
  real(dp),parameter :: epsArg_p = 1.0e-15_dp        !< Argument tolerance

  integer, parameter :: maxFunc_p = 22 !< Number of function types
  ...
  integer, save :: dbgFunc = 0 !< File unit number of debug print

  integer, parameter :: nSpln_p = 6 !< Max. number of spline functions in core

  !> @brief Data type for spline functions.
  type SplineData
     integer           :: N    !< Number of control points of the spline
     real(dp), pointer :: X(:) !< Argument values at the spline control points
     real(dp), pointer :: Y(:) !< Function values at the spline control points
     real(dp), pointer :: DERIV(:,:) !< First and second derivatives
  end type SplineData

  type(SplineData), save :: spl(nSpln_p)      !< Internal spline data buffer
  integer         , save :: last(nSpln_p) = 0 !< Identifier for splines in core
...
contains
  ...
  !>
  !> @param[in] ikf Function identification number
  !> @param[in] ifunc Integer function input parameters
  !> @param[in] rfunc Real function input parameters
  !> @param[in] x Function argument
  !> @param[out] fn Value of function
  !> @param[out] ierr Error indicator
  !>
  !> @callgraph
  !>
  !> @author Ole Ivar Sivertsen SINTEF avd. for maskinkonstruksjon
  !>
  !> @date 1989
  ...
  subroutine funcValue1 (ikf,ifunc,rfunc,x,fn,ierr)
    ...
    case(COMPL_SINUS_p)
       FN = FUNC2(rfunc,X,0.0_dp,.false.)

    case(DELAYED_COMPL_SINUS_p)
       FN = FUNC2(rfunc,X,0.0_dp,.true.)
...
601    format('funcValue: ',A15,', IKF =',I6,'    F =',1P,E12.5)
  ...
  !!============================================================================
  !> @brief Evaluates an (possibly) multi-argument explicit function.
  !>
  !> @param[in] ikf Function identification number
  !> @param[in] ifunc Integer function input parameters
  !> @param[in] rfunc Real function input parameters
  !> @param[in] x Function arguments
  !> @param[out] fn Value of function
  !> @param[out] ierr Error indicator
  !>
  !> @callgraph
  ...
  subroutine funcValue2 (ikf,ifunc,rfunc,x,fn,ierr)
    ...
    if (ifunc(1) == USER_DEFINED_p) then
       ...
       case (COMPL_SINUS_p)
          FN = FUNC2(rfunc,X(1),X(2),.false.)
       case (DELAYED_COMPL_SINUS_p)
          FN = FUNC2(rfunc,X(1),X(2),.true.)
       case (WAVE_SINUS_p)
...
600    format('funcValue: ',A15,', IKF =',I6,' X, F =',1P6E12.5)
  ...
  !>
  !> @param[in] ikf Function identification number
  !> @param[in] iorder Derivative order
  !> @param[in] ifunc Integer function input parameters
  !> @param[in] rfunc Real function input parameters
  !> @param[in] x Function argument
  !> @param[out] fn Derivative of function
  !> @param[out] ierr Error indicator
  !>
  !> @callgraph
  !>
...
600    format('funcDerivative1: ',A,', IKF =',I6,' X, dF/dX =',1P2E12.5)
  ...
  !!============================================================================
  !> @brief Evaluates the derivative of a multi-argument explicit function.
  !>
  !> @param[in] ikf Function identification number
  !> @param[in] iorder Derivative order
  !> @param[in] ifunc Integer function input parameters
  ...
  !!============================================================================
  !> @brief Integrates an explicit function from 0.0 to @a x.
  !>
  !> @param[in] ikf Function identification number
  !> @param[in] iorder Integration order
  !> @param[in] ifunc Integer function input parameters
  !> @param[in] rfunc Real function input parameters
  !> @param[in] x Function arguments
  !> @param[out] fn Integral of function
  !> @param[out] ierr Error indicator
  !>
  !> @callgraph
  !>
...
600    format('funcIntegral: ',A,', IKF, IORDER =',I6,I2, &
  ...
  !!============================================================================
  !> @brief Query function telling if a function can be integrated explicitly.
  !>
  !> @callgraph
  !>
  !> @author Knut Morten Okstad
  ...
  !!============================================================================
  !> @brief Prints an error message when function argument is out of range.
  ...
  !************************************************************  FUNC1   *
  !***********************************************************************
  !> @brief Simple sinusoidal function.
  !>
  !> @details The following is from the original FORTRAN-77 documententation:
  !> @code
  !> ***********************************************************************
  !> *
  ...
  !************************************************************  FUNC2   *
  !***********************************************************************
  !> @brief Complex sinusoidal function composed of two sine waves.
  !>
  !> @details The following is from the original FORTRAN-77 documententation:
  !> @code
  !> ***********************************************************************
  ...
  !> @callergraph
  !>
  !> @author Sjur Lassesen SINTEF avd. for maskinkonstruksjon
  !>
  !> @date 1986

  function FUNC2 (RFUNC,T,X,useTmin)
  ...
  !************************************************************  FUNC4   *
  !***********************************************************************
  !> @brief Complex sinusoidal function composed of many sine waves.
  !>
  !> @details The following is from the original FORTRAN-77 documententation:
  !> @code
  !> ***********************************************************************
  ...
  !************************************************************  FUNC5   *
  !***********************************************************************
  !> @brief Servo simulator function
  !>
  !> @details The following is from the original FORTRAN-77 documententation:
  !> @code
  !> ***********************************************************************
...
915 IERR = IERR - 1
  ...
  !!============================================================================
  !> @brief Deallocates all spline function data.
  ...
  !************************************************************  SPLIN3  *
  !***********************************************************************
  !> @brief Computes a spline approximation from a set of points.
  !>
  !> @details The following is from the original FORTRAN-77 documententation:
  !> @code
...
3000 format('Error :   subroutine SPLIN3 ERROR NO',I3,' ***',1P2E25.14)
  ...
  !************************************************************  FUNC 8  *
  !***********************************************************************
  !> @brief Evaluates the streamline wave profile at point @a X and time @a T.
  !>
  !> @param[in] RFUNC Wave data, output from wavefunctionsmodule::initfunc8
  !> @param[in] T Current time
  !> @param[in] X Current position
  !> @param[out] IERR Error indicator
  !> @return Current wave height above @a z = 0
  !>
  !> @callergraph
  !>
  !> @author Knut Morten Okstad
  ...
  !************************************************************  FUNC 9  *
  !***********************************************************************
  !> @brief Evaluates the embedded wave profile at point @a X and time @a T.
  !>
  !> @param[in] IFUNC Wave data, output from wavefunctionsmodule::initfunc9
  !> @param[in] RFUNC Wave data, output from wavefunctionsmodule::initfunc9
  !> @param[in] T Current time
  !> @param[in] X Current position
  !> @param[out] IERR Error indicator
  !> @return Current wave height above @a z = 0
  ...
  !************************************************************  FUNC10  *
  !***********************************************************************
  !> @brief Evaluates a piece-wise linear function at the given point.
  !>
  !> @param[in] IKF Function identification number
  !> @param[in] IFUNC Integer function input parameters
  !> @param[in] RFUNC Real function input parameters
  !> @param[in] X Function argument
  !> @param[out] ierr Error indicator
  !> @return Function value at point @a X
  !>
  !> @callergraph
  !>
  !> @author Knut Morten Okstad
  ...
  !!============================================================================
  !> @brief Evaluates the derivative of a piece-wise linear function.
  !>
  !> @param[in] IKF Function identification number
  !> @param[in] IFUNC Integer function input parameters
  !> @param[in] RFUNC Real function input parameters
  !> @param[in] X Function argument
  !> @param[out] ierr Error indicator
  !> @return Function derivative at point @a X
  !>
  !> @callergraph
  !>
  !> @author Knut Morten Okstad
...
900    format('Evaluating derivative at a discontinuity, x =',1PE13.5)
  ...
  !!============================================================================
  !> @brief Integrates a piece-wise linear function from 0.0 to @a X.
  !>
  !> @param[in] IKF Function identification number
  !> @param[in] IFUNC Integer function input parameters
  !> @param[in] RFUNC Real function input parameters
  !> @param[in] X Function argument, upper range of integration domain
  !> @param[in] IORD Integration order (0, 1, or 2)
  !> @param[out] ierr Error indicator
  !> @return Function integral at point @a X
  !>
  !> @callergraph
  !>
  !> @author Bjorn Haugen
  ...
  !> @callergraph
  !>
  !> @author Torleif Iversen (OIS) SINTEF avd. for reguleringsteknikk
  !>
  !> @date 1989
  ...
  !!============================================================================
  !> @brief Integrate a ramp function from 0.0 to @a X.
  !>
  !> @param[in] IORD   Integration order (1 or 2)
  !> @param[in] X      End point of integration
  !> @param[in] OFFSET Function value offset
  !> @param[in] SLOPE  Ramp slope
  !> @param[in] X0     Start point for ramp
  !> @param[in] X3     End point for ramp
  !> @return Function integral at point @a X
  !>
  !> @callergraph
  !>
  ...
  !> @callergraph
  !>
  !> @author Terje Rolvag (OIS) SINTEF avd. for reguleringsteknikk
  !>
  !> @date 1989
  ...
  !************************************************************  FUNC18  *
  !***********************************************************************
  !> @brief Calls a C-function to read and interpolate a function from a file.
  !>
  !> @callergraph
  !>
  !> @author Tommy Jorstad / Knut Morten Okstad
  !>
  !> @date 14 Jun 2002
  ...
  !!============================================================================
  !> @brief Calls a C-function to read and integrate a function from a file.
  !>
  !> @callergraph
  !>
  !> @author Knut Morten Okstad
  ...
  !************************************************************  FUNC19  *
  !***********************************************************************
  !> @brief Calls a C-function to get function value of an expression function.
  !>
  !> @callergraph
  !>
  ...
  !!============================================================================
  !> @brief Calls a C-function to get derivative of an expression function.
  !>
  !> @callergraph
  !>
  !> @author Knut Morten Okstad
  ...
  !************************************************************  FUNC21  *
  !***********************************************************************
  !> @brief Calls a C-function to get function value of a user-defined function.
  !>
  !> @callergraph
  !>
  !> @author Knut Morten Okstad
  ...
  !!============================================================================
  !> @brief Calls a C-function to get derivative of a user-defined function.
  !>
  !> @callergraph
  !>
  !> @author Knut Morten Okstad
  ...
  !#######################################################   FUNC22   ####
  !#######################################################################
  !> @brief Smooth trajectory function.
  !>
  !> @details The following is from the original FORTRAN-77 documententation:
  !> @code
  !> ***********************************************************************
  ...
  !> @callergraph
  !>
  !> @author Hans Petter Hildre 503914, NTH Institutt for maskinkonstruksjon
  !>
  !> @date 27 Mar 1990
